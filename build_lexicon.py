#!/usr/bin/env python3
"""Bake canonical.json into docs/lexicon.js so the piece runs from file:// with no server.
...
source of truth. Re-run this whenever canonical.json changes:
    ...
    python3 build_lexicon.py           # validate, then write docs/lexicon.js
    python3 build_lexicon.py --check   # validate only

The build refuses to write docs/lexicon.js if canonical.json fails validation.
...
def main():
    ...
    if errors:
        print(f"canonical.json failed validation ({len(errors)} problem(s)); {OUT} not written:",
              file=sys.stderr)
        for err in errors:
    ...
    if "--check" in sys.argv[1:]:
        ...
        return
    print(f"Wrote {OUT} with {build(data)} entries.")
